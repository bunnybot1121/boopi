    ...
    def run(self):
        while True:
            try:
                ...
                if not spoken_buffer.strip() and not self._interrupted:
                    log.warning("No spoken text generated by LLM stream. Emitting friendly fallback to release thinking lock.")
                    self.response_started.emit("happy")
                    self.response_chunk.emit("I'm right here! How can I help you?")
