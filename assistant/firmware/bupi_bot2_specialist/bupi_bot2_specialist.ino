...
float readUltrasonic() {
  ...
  // Temperature-compensated speed of sound (331.3 + 0.606 * T) in cm/us
  float speedOfSound = (331.3 + 0.606 * tempC) * 100.0 / 1000000.0;
  float rawCm = durationUs * speedOfSound / 2.0;
...
float readMQ2Gas() {
  // 8-sample oversampling to suppress ESP32 ADC switching noise
  long sum = 0;
  for (int i = 0; i < 8; i++) {
  ...
  // Real-world calibrated PPM conversion:
  // Clean ambient indoor air produces ~400-1500 ADC (preheat/sensor baseline).
  // In real life, clean air concentration of CO/LPG/smoke is 25-50 ppm.
  // Smoke or direct gas exposure pulls ADC above 2200-3500 (>150 ppm).
  float newPpm;
  if (avgAdc <= 1400.0) {
    // Clean ambient room air baseline
    newPpm = 25.0 + (avgAdc / 1400.0) * 20.0; // 25.0 to 45.0 ppm
  } else if (avgAdc <= 2200.0) {
    // Mild variation / ambient elevation
    newPpm = 45.0 + ((avgAdc - 1400.0) / 800.0) * 55.0; // 45.0 to 100.0 ppm
  } else if (avgAdc <= 3200.0) {
    // Definite gas or smoke presence
    newPpm = 100.0 + ((avgAdc - 2200.0) / 1000.0) * 250.0; // 100.0 to 350.0 ppm
  } else {
    // Critical hazard / dense smoke
    newPpm = 350.0 + ((avgAdc - 3200.0) / 895.0) * 450.0; // 350.0 to 800.0 ppm
  }

  // Exponential Moving Average filter (alpha = 0.20) for smooth, stable telemetry
  static float smoothedGas = 35.0;
  smoothedGas = 0.80 * smoothedGas + 0.20 * newPpm;
  return smoothedGas;
}

// DHT reading state tracking
static bool dhtReadingValid = false;
...
void readMPUAndEnvironment() {
  ...
  if (Wire.available() >= 14) {
    ...
    // Only estimate from MPU die if DHT22 is not available or timed out (>10s)
    if (!dhtReadingValid || (now - lastValidDhtTime > 10000)) {
      float mpuTemp = (raw_temp / 340.0) + 36.53;
      // MPU6050 die has ~18.5°C internal dissipation offset above ambient
      float estimatedAmbient = mpuTemp - 18.5;
      if (estimatedAmbient >= 16.0 && estimatedAmbient <= 36.0) {
        tempC = estimatedAmbient;
      } else {
        tempC = 24.5; // Calibrated safe indoor room baseline
      }
    }
  ...
  if (now - lastDhtReadTime >= 2000) {
    ...
    if (!isnan(dhtT) && dhtT > -10.0 && dhtT < 60.0) {
      tempC = dhtT;
      dhtReadingValid = true;
      lastValidDhtTime = now;
    }
    if (!isnan(dhtH) && dhtH >= 0.0 && dhtH <= 100.0) {
      humidityPct = dhtH;
    }
  }
  #endif
...
String buildTelemetryJson() {
  ...
  const char* gasStatus = (gasPpm < 60.0) ? "CLEAN" : ((gasPpm < 150.0) ? "NORMAL" : ((gasPpm < 300.0) ? "WARNING" : "DANGER"));
  float heatIndex = -8.784695 + 1.61139411 * tempC + 2.338549 * humidityPct - 0.14611605 * tempC * humidityPct;

  char telemBuf[640];
  snprintf(telemBuf, sizeof(telemBuf),
    "{\"type\":\"telemetry\",\"bot_id\":\"%s\",\"robot_id\":\"%s\",\"device\":\"%s\",\"role\":\"%s\",\"ip\":\"%s\",\"ssid\":\"%s\",\"wifi_rssi\":%d,\"distance_cm\":%.1f,\"gas_ppm\":%.1f,\"gas_status\":\"%s\",\"temp_c\":%.1f,\"humidity\":%.1f,\"heat_index\":%.1f,\"pitch\":%.1f,\"roll\":%.1f,\"heading\":%.1f,\"ax\":%.2f,\"ay\":%.2f,\"az\":%.2f,\"gz\":%.1f,\"steps\":%lu,\"dist_m\":%.2f,\"obstacle\":%s,\"moving\":%s}",
    BOT_ID,
    BOT_ID,
    BOT_NAME,
    BOT_ROLE,
