@echo off
cd /d "%~dp0"
title Flash BUPI Bot 2 (Specialist)
echo ========================================================
echo     [FLASHING] BUPI BOT 2 (SPECIALIST) FIRMWARE
echo ========================================================
echo.
echo Please ensure Bot 2 is connected via USB cable.
echo.
set PORT=COM3
if not "%~1"=="" set PORT=%~1
...
arduino-cli compile --upload -p %PORT% --fqbn esp32:esp32:esp32 firmware/bupi_bot2_specialist
if %errorlevel% neq 0 (
    echo.
    echo [ERROR] Upload failed on %PORT%.
    echo Check Device Manager for the correct CP210x COM port.
    echo Usage: Flash_Bot2.bat COMx
    pause
    exit /b 1
)
echo.
echo [SUCCESS] Bot 2 (Specialist) Flashed successfully!
pause
